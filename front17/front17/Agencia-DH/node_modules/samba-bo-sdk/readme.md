...
```ts
const client = createSambaBO<Schema>('https://samba_bo.server');
```
...
```ts
const client = createSambaBO<Schema>('https://samba_bo.server').with(rest()).with(graphql());
...
```ts
const client = createSambaBO<Schema>('https://samba_bo.server').with(rest()).with(authentication('json'));
...
```ts
const client = createSambaBO<Schema>('https://samba_bo.server').with(rest()).with(staticToken('super-secure-token'));
...
The `realtime()` extension allows you to work with a SambaBO REST WebSocket.

Subscribing to updates:

```ts
const client = createSambaBO<Schema>('https://samba_bo.server').with(
	realtime({
		authMode: 'public',
	}),
);
